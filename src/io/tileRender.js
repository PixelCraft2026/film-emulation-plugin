 ...
 */
import { processHalation, lowResScale, applyMatrix3, resolveSigmaParams } from '../core/index.js';
...
export function processTiledWithTrc(input, params, trc, opts = {}) {
  ...
  const algoOpts = { luma: [0.2126, 0.7152, 0.0722], compact: true };
  ...
  if (width * height <= tileThreshold) {
    ...
    return { width, height, rgb: encodeFromLinear(out.rgb, outputTrc), alpha: out.alpha };
  }
  ...
  return { width, height, rgb: out, alpha: outAlpha };
}
