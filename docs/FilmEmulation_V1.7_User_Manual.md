...
适用版本：Film Emulation package 1.7.0 / V1.7 Public Beta 1
Halation 引擎版本：1.5.1
Photoshop 最低版本：23.3
测试平台范围：Windows 10/11；macOS 未验证，不纳入本 Beta 的支持声明
最后核对：2026-09-05（本次重点核对 Defringe、Bloom 与 Highlight Protection 实现）

> 本手册文件名为 `FilmEmulation_V1.7_User_Manual.md`；文档标题和内容以 Film Emulation V1.7 Public Beta 1 为准。本轮 Windows Photoshop 实机验收已由项目维护者汇总签核，但它仍是公开测试版，不等于正式稳定版。

## 目录

- [1. 插件用途与边界](#1-插件用途与边界)
- [2. 推荐工作流程](#2-推荐工作流程)
- [3. V1.7 Public Beta 1 界面布局与共享胶片设置](#3-v17-public-beta-1-界面布局与共享胶片设置)
  - [页面结构与节点开关](#31-页面结构与节点开关)
  - [面板身份与界面语言](#32-面板身份与界面语言)
  - [参数页与预览模式](#33-参数页与预览模式)
  - [Film stock 共享设置](#34-film-stock-共享设置)
  - [效果区域与 Luma mask](#35-效果区域与-luma-mask)
- [4. Halation：原理、预设与调节](#4-halation原理预设与调节)
  - [完整处理顺序与 Halation Pipeline](#41-完整处理顺序与-halation-pipeline)
  - [色彩与透明度](#42-色彩与透明度)
  - [高光提取与曝光重建](#43-高光提取与曝光重建)
  - [光谱源场](#44-光谱源场)
  - [强种子邻域扩张](#45-强种子邻域扩张)
  - [三瓣 PSF](#46-三瓣-psf)
  - [局部光晕与全局红层](#47-局部光晕与全局红层)
  - [合成与输出](#48-合成与输出)
  - [相关节点在处理链中的位置](#49-相关节点在处理链中的位置独立调节详见第-5-章)
  - [V1.7 完整处理链的执行后端](#410-v17-完整处理链的执行后端)
  - [Halation 预设说明](#411-halation-预设说明)
  - [Halation 参数详细说明](#412-halation-参数详细说明)
  - [参数之间最重要的关系](#413-参数之间最重要的关系)
  - [典型调参配方](#414-典型调参配方)
- [5. Defringe、Bloom 与 Highlight Protection](#5-defringe-bloom-与-highlight-protection)
  - [Defringe（去色边）](#51-defringe去色边)
  - [Bloom（泛光）](#52-bloom泛光)
  - [Highlight Protection（高光保护）](#53-highlight-protection高光保护)
  - [三个节点的效果区域（Luma mask）](#54-三个节点的效果区域luma-mask)
- [6. Film Resolution / MTF](#6-film-resolution--mtf)
- [7. Film Grain](#7-film-grain)
- [8. Preview、Apply 与非破坏性行为](#8-previewapply-与非破坏性行为)
- [9. 位深、工作空间与输出注意事项](#9-位深工作空间与输出注意事项)
- [10. 常见问题](#10-常见问题)
- [11. 推荐调参顺序速查](#11-推荐调参顺序速查)

## 1. 插件用途与边界

Film Emulation 是 Photoshop 静态图像胶片物理效果插件。当前插件包和 graph 的语义版本均为 1.7.0，按固定物理顺序排列：
...
- 当前输入必须是 RGB 像素层。智能对象、文字、调整层或组需要先进入其内容编辑状态，或先栅格化为像素层。
...
3. 需要时先打开 Defringe，默认使用 100%/Actual 检查边缘；先用 `Amount`、`Radius` 建立校正范围，再调 `Chroma threshold`、`Chroma softness` 和 `Edge sensitivity`。它只处理局部紫边/绿边，不是全局去饱和。
...
5. 打开 Bloom 后先调 `Threshold (EV)`、`Softness (EV)` 和按完整画面对角线换算的 `Radius (% diagonal)`，再用 `Amplify`、`Saturation`、`Save lights` 塑形；Bloom 默认使用 Fit 预览。
6. 如需保护高光，先确认 Bloom 已启用，再在 Bloom 域打开 Highlight Protection。它只衰减最近的 Bloom contribution，不会压低原始高光；没有前置 Bloom 时保持输入不变并显示缺失提示。
...
Public Beta 1 是第一个公开版本。发布包只有 `FilmEmulation.ccx`，插件 ID 为 `com.cheukwing.filmemulation`；当前没有外部旧 ID 用户需要迁移，因此面板不提供旧状态导入/导出按钮，也不需要安装迁移桥。

首次运行会根据 Photoshop/系统 locale 选择中文或英文。底栏的 `界面语言 / Language` 可即时切换，选择会保存为插件全局偏好；它不写入文档 schema，不改变 graph、颗粒 seed、格式参数或源/目标图层绑定。标准 CCX 只携带 scalar `film_core.wasm`；SIMD 工件仅保留给 Node QA，不会被 Photoshop 读取、实例化或分发。

## 3. V1.7 Public Beta 1 界面布局与共享胶片设置
...
当前插件在 Photoshop 中显示为 `Film Emulation`，唯一面板 entrypoint 为 `filmEmulationPanel`。底栏提供 `中文 / English` 选择；首次运行按宿主/系统语言选择，手动选择会在不同会话之间保留，但不会写入文档状态。
...
- 首次进入 100% 模式时，视窗定位于源图层中央。按住预览拖动可检查其他区域；方向键每次移动 64px，按住 Shift 时每次移动 256px。
...
`ISO` 范围为 25–3200，默认 250。提高 ISO 会同时增大颗粒尺度与强度，并轻微降低目标分辨率。Film format（胶片格式）和 ISO 是共享物理设置；在 Resolution 页修改后，Grain 页会使用相同值，反之亦然。

### 3.5 效果区域与 Luma mask

每个效果的 Luma mask 能力保留，但默认置于 `Advanced` 中。UI 不再直接使用内部名 `Luma mask`：普通节点显示 `Effect area（效果区域）`，Halation/Bloom 显示 `Halation output area（胶片光晕输出区域）` / `Bloom output area（泛光输出区域）`，Highlight Protection 显示 `Protection area（保护区域）`。`Apply to（应用于）: Entire image（整张图像）` 时整帧生效；选择 `Exposure range（曝光范围）` 后才显示 `Lower bound（下限）`、`Upper bound（上限）`、`Edge softness（边缘柔和度）` 和 `Inside/Outside EV range（EV 范围内/外）`。Bloom output area 只限制扩散结果落在哪里，不改变哪些高光被提取成 Bloom 光源。mask 默认关闭，不会改变现有图像；启用曝光范围后，当前节点的最小 engine version 会提升为 1.7.0。

## 4. Halation：原理、预设与调节

本章集中说明 Halation 的物理流程、预设、全部参数、参数关系和典型配方。Defringe、Bloom 与 Highlight Protection 虽然在完整处理链中与 Halation 相邻，但它们的独立用法与参数说明统一放在第 5 章。

### 4.1 完整处理顺序与 Halation Pipeline
...
- 面板底图最长边为 1024 px，按 sRGB 编码显示。
- 光源提取使用最长边为 2048 px 的高分辨率代理，扩散在 1024 px 显示尺度上进行。
- Apply 在源图层分辨率处理。
- 完全透明像素不产生光晕；半透明像素按 alpha 比例贡献光能。
- 输出 alpha 原样保留。

Defringe、Halation、Bloom 和 Highlight Protection 都在 canonical linear sRGB 中运行；显示编码 RGB 不会直接参与卷积或阈值判断。透明像素的隐藏 RGB 不会成为 Bloom 光源，Bloom 与 Halation 的所有正向贡献都按 alpha 缩放。
...
为了让 8/16 位输入中被限制在 0–1 的高光仍保留足够动态范围，插件把阈值 T 到数字白点 1 重建为强光坐标 U：
...
因此被裁剪为白色的高光 U 仍为 1，而刚刚越过阈值的窗户仍接近 0。`Source Impact` 再以 `1 + 1.5×Impact` 的指数拉开弱光和强光。
...
因此“蓝天中的白灯”“略偏冷的白灯”和“纯蓝 LED”会得到不同的判定结果：前两者可以形成红晕，只有高色纯度的蓝/青 LED 被严格拒绝。
...
3. 每个候选还会按自身色纯度复核：邻近的白色、暖色或低饱和冷白光学 glow 可以被纳入 Halation 源场，高饱和蓝/青像素会被拒绝。
...
`Source Interior Protection` 另有一条自适应路径：源场局部占用率和较宽的环境亮度共同区分“大面积亮反射面”与“黑夜中的紧凑灯芯”。对白衣、灯箱等连续亮面以及实际灯芯本体，插件只保留越过外轮廓的残差；在暗环境中，经复核的低阈值 optical glow 会局部放开保护，使原始光学 glow 与外缘红晕保持连续。明亮、高饱和蓝/青 LED 还有目标侧保护，不会被附近其他白灯的红晕覆盖成紫红色；较暗蓝天不会触发该保护，白灯仍能在蓝天背景上形成红晕。

`Global Red Diffusion` 是另一条独立路径。它只接收超过 `Global Source Level` 的强光，以 `max(12px, 4×Sigma)` 的宽尺度扩散，并主要落在中间调区域。密集源采用软饱和聚合，避免窗户阵列无限叠加成红雾。
...
Additive 直接增加正光能。Screen 在 0–1 范围内按剩余高光余量增加；在 HDR >1 时使用随亮度递减但始终为正的增益，禁止把原始高光反向变暗。
...
### 4.9 相关节点在处理链中的位置（独立调节详见第 5 章）

#### Defringe

Defringe 先把线性 RGB 变换到 YCoCg，只对 Cg 做局部边缘校正，保留亮度 Y 与红蓝差异 Co。边缘门控由 Y 的局部变化和 Cg 的色差共同决定，因此主要处理紫边/绿边，不会把整幅图洗成中性灰。`Amount=0` 或 `Edge sensitivity=0` 时逐个 Float32 样本恒等，alpha 始终引用原输入。
...
半径按完整图像对角线和 preview scale 换算，而不是按当前分带尺寸换算。Fast 与 Quality 保持相同三瓣、半径和总能量，仅改变数值核。Bloom 会把 `bloomBase`（节点输入）和已应用 Saturation、Save lights、Amplify 的 `bloomContribution`（最终正向贡献）放入 transient bus，供后面的 Highlight Protection 使用；HDR 高光不会因 Bloom 变暗。

#### Highlight Protection

Highlight Protection 不重新提取高光，也不修改源图。它只对最近一个前置 Bloom 的 `bloomContribution` 按亮度阈值衰减，并保持 `bloomBase` 与原始输入的关系。若图中没有可用 Bloom transient，节点保持输入不变，状态会记录 `missingBloomContribution`；节点不会被静默删除。
...
### 4.12 Halation 参数详细说明

以下顺序按照算法流程分组，而不是完全照搬面板排列顺序。
...
- 主 Threshold 控件的 +3–+4 区间对应 HDR 头部肩部：它从 +3 EV 连续加速延伸到更高的有效曝光阈值，并在 +4 进入“无光源”；Background Threshold 仍按显示值直接换算。
- 切换单位时，当前 Threshold 和 Background Threshold 数字不会自动换算，必须重新调整。

#### 4.12.4 Highlight Extraction（高光提取）
...
#### 4.12.5a Red-Layer Threshold Bias（红色感光层阈值偏移）
...
- 调高：弱源被相对压低，最亮光源更突出。
- 1：强弱光的非线性分离最明显，适合灯牌和被裁剪为白色的高光。

如果出现“所有窗户强度差不多”，提高 Source Impact；如果只有最亮灯芯有反应，适当降低该参数。

#### 4.12.9 Hue Response（色相响应）
...
#### 4.12.10 Strong Source Level（强光源级别）
...
- 8/16 位中被裁剪为白色的高光 U=1，因此 Level 小于 1 时仍可被识别。

Strong Source Level 不决定高光是否进入基础源场；那是 Threshold 的职责。

#### 4.12.11 Strong Source Expansion（强光源扩张）
...
如果高亮只出现细红线而没有扎实肩部，提高 Strong Source Expansion；如果弱窗连成片，先提高 Strong Source Level，而不是只降低 Expansion。

#### 4.12.12 Halation Amplify（胶片光晕增益）
...
#### 4.12.13 Background Threshold（背景阈值）
...
#### 4.12.14 Background Softness（背景柔和度）
...
#### 4.12.15 Blue Compensation（蓝色补偿）
...
- 同时会适度提高冷背景上的 Color Density 可见能量。
- 过大：冷色背景上的红晕可能比暖背景更抢眼。

“蓝色背景中的白灯无红晕”应提高 Blue Compensation；“纯蓝灯出现红晕”应提高 Hue Response，而不是降低 Blue Compensation。

#### 4.12.16 Sigma Units（Sigma 单位）
...
#### 4.12.18 PSF Smoothness（PSF 平滑度）
...
#### 4.12.20 Sigma Ratio R / G / B（Sigma 比例 R / G / B）
...
#### 4.12.21 Red Shift R / G / B（红移 R / G / B）
...
#### 4.12.22 Center Attenuation（中心衰减）
...
如果光晕像柔焦 Bloom，可适当提高 Center Attenuation；如果灯芯外围出现空心红圈，则降低该参数。

#### 4.12.22a Source Interior Protection（光源内部保护）
...
人物白衣、大面积高亮或灯芯内部出现粉红块时，提高 Source Interior Protection。当前算法通过保留原始底图光源并只在外围加入红层扩散，避免保护核心被误解为亮度空洞；需要无 Remjet 式全面实体核芯时再降低。Neutral 默认为 1，CineStill 800T 默认为 0。

#### 4.12.23 Strong Core（强光核心）
...
它与 Center Attenuation 配合控制强弱光源的中心保留量：Center Attenuation 决定普通源扣除量，Strong Core 决定强源能保留多少实体感。

#### 4.12.24 Global Source Level（全局光源级别）
...
#### 4.12.25 Global Red Diffusion（全局红色扩散）

范围：0–1。阶段：独立宽红层。

- 0：关闭全局红层，只保留局部 Halation。
- 调大：在强光周围增加更宽、以红色为主、主要落在中间调的扩散。
- 不是白色 Bloom；通道比例固定为红色主导。
- 宽半径约为 `max(12px, 4×Sigma)`。
- 过大：画面对比度下降，多个强光可能形成大面积红色雾感。

建议先用局部 PSF 调出正确形状，最后只添加少量 Global Red Diffusion。

#### 4.12.26 Halation Color Density（胶片光晕色彩密度）
...
如果光晕宽度正确但仍像淡红色透明雾，提高 Halation Color Density；如果已经出现纯红剪切，先降低该参数，再考虑降低 Amplify。

#### 4.12.27 Blend Mode（混合模式）
...
#### 4.12.29 Highlight Rolloff（高光滚降）
...
#### 4.13.1 Threshold（阈值）、Source Impact（光源影响）、Strong Source Level（强光源级别）
...
#### 4.13.2 Sigma（半径 Sigma）、PSF Smoothness（PSF 平滑度）、Red Tail（红色尾部）
...
#### 4.13.3 Halation Amplify（胶片光晕增益）、Strength（强度）、Halation Color Density（胶片光晕色彩密度）
...
#### 4.13.4 Background Threshold（背景阈值）、Blue Compensation（蓝色补偿）、Hue Response（色相响应）
...
#### 4.13.5 Center Attenuation（中心衰减）、Strong Core（强光核心）、Strong Source Expansion（强光源扩张）、Source Interior Protection（光源内部保护）
...
## 5. Defringe、Bloom 与 Highlight Protection

这三个 V1.7 节点解决的是不同问题。它们虽然都属于胶片物理效果，但解决的不是同一种 Glow 问题：

| 节点 | 所在阶段 | 主要作用 | 是否直接压低原始高光 |
|---|---|---|---|
| Defringe（去色边） | Halation 之前 | 在局部亮度边缘校正紫边/绿边 | 否 |
| Bloom（泛光） | Halation 之后 | 从高光提取正向光能，并用三瓣 PSF 向外扩散 | 否，只增加贡献 |
| Highlight Protection（高光保护） | Bloom 之后 | 衰减 Bloom contribution，保留更扎实的高光芯 | 否 |

新建 graph 会创建这三个节点，但默认只启用 Halation；节点开关控制是否执行，左侧标签/页签只负责切换参数页，不会改变实际物理顺序。当前顺序固定为 `Defringe → Halation → Bloom → Highlight Protection → Film Resolution → Film Grain`。

### 5.1 Defringe（去色边）

#### 它处理什么

Defringe 处理的是高反差边缘常见的紫边、绿边或品红/绿色局部色差，不是全局去饱和，也不是锐化。它不改变像素位置，不生成光晕；处理完成后仍保留原始 alpha。

节点在 canonical linear sRGB 中工作。实现先把每个像素从线性 RGB 变换到 YCoCg：
...
随后对 Y 和 Cg 做相同半径的局部模糊，计算亮度边缘 `|Y - Yblur|` 与色度差异 `|Cg - Cgblur|`。只有两者同时达到门控条件时，才把 Cg 向邻域平均值平滑，Y 和 Co 保持不变，再反变换回 RGB。因此它主要压低绿↔品红轴上的边缘色差，不会把整张图洗成灰，也不会主动改变红↔蓝差异。

Fast 使用较快的 box blur，Quality 使用可分离 Gaussian blur；两者使用相同的边缘/色度门控和混合逻辑。`Amount=0` 或 `Edge sensitivity=0` 时，Defringe 是逐 Float32 样本恒等的。

#### 参数速查

| 参数 | 范围 / 默认值 | 作用 |
|---|---|---|
| `Amount`（数量） | 0–1 / 0.65 | Cg 向局部平均值校正的最大混合量；0 为关闭校正 |
| `Radius (px)`（半径） | 0.5–4 / 1.5 | 判断局部边缘色差的邻域半径；越大，作用范围越宽 |
| `Chroma threshold`（色度阈值） | 0–1 / 0.08 | `|Cg-Cgblur|` 达到多少后才认为可能是色边；越低越敏感 |
| `Chroma softness`（色度柔和度） | 0.01–0.5 / 0.12 | 色度门控的过渡宽度；越大越渐进，越小越明确 |
| `Edge sensitivity`（边缘灵敏度） | 0–2 / 1 | 亮度边缘门控的灵敏度；越高越容易纳入边缘，0 为恒等 |

#### 怎么调

1. 先切换到 100%/Actual 预览，在细线、树枝、建筑轮廓或逆光人物边缘寻找紫/绿边。
2. 先把 `Amount` 放在 0.4–0.7、`Radius` 放在 1–2 px 附近；画面仍有明显色边时，再降低 `Chroma threshold`。
3. 如果校正过渡太硬，提高 `Chroma softness`；如果彩色细节也被削弱，提高 `Chroma threshold` 或降低 `Edge sensitivity`。
4. 如果影响范围过宽，优先减小 `Radius`，其次减小 `Amount`。Defringe 位于 Halation 之前，先清理的残余紫/绿边不会被后续 Halation 误当成发光体。

#### 效果区域

Defringe 的 Advanced 面板中的 `Effect area` 使用共享 Luma mask。选择 `Entire image` 时整张图可被校正；选择 `Exposure range` 时，插件按线性 sRGB Rec.709 亮度换算 EV，再把输入与 Defringe 结果按遮罩混合。遮罩不会改变 alpha，也不会把 HDR 值裁到 0–1。

### 5.2 Bloom（泛光）

#### 它如何生成泛光

Bloom 是独立于 Halation 的高光散射。按照默认物理顺序，它接收经过 Halation 处理的节点输入，从当前输入的 RGB 高光中提取光源，再把正向贡献加回输入：

```text
T       = 0.18 × 2^ThresholdEV
gateEnd = T × 2^SoftnessEV
gate    = smoothstep(T, gateEnd, max(R, G, B))
output  = input + bloomContribution
```

提取时负的 HDR 残差会被排除，透明像素的隐藏 RGB 不会成为光源。`Saturation` 决定提取出的光是趋向中性白还是保留原色，之后使用固定的正向三瓣 PSF：
...
三个权重为正且总和为 1，分别负责紧实光芯、中距离肩部和远端尾部。Bloom 的 `Radius (% diagonal)` 按完整图像对角线换算，而不是按当前分带尺寸换算：

```text
radiusPx = Radius / 100 × hypot(fullWidth, fullHeight) × previewScale
```

因此 `0.7` 表示完整画面对角线的 0.7%，不是 0.7 px；预览缩放只影响代理图上的对应像素半径，Apply 使用完整图像尺度。Fast 与 Quality 保持相同的三瓣、半径和正向加光语义，只改变模糊的数值近似与宽核的采样方式。

Bloom 会把节点输入保存为 `bloomBase`，把经过色彩调整、保留高光和增益处理后形成的最终正向 `bloomContribution` 保存到 transient bus，供后面的 Highlight Protection 使用。`Amplify=0` 时 Bloom 输出逐样本等于输入，但仍会发布零贡献 transient。

#### 参数速查

| 参数 | 范围 / 默认值 | 作用 |
|---|---|---|
| `Threshold (EV)`（阈值） | -2–8 / 2 | 高光提取开始的亮度；相对中性灰 0.18 的 EV，越低越容易产生泛光 |
| `Softness (EV)`（柔和度） | 0.1–4 / 1 | 从阈值过渡到完整提取的 EV 宽度；它控制门控渐变，不是空间模糊半径 |
| `Radius (% diagonal)`（半径） | 0.05–5 / 0.7 | 三瓣 PSF 的整体扩散尺度，按完整画面对角线百分比计算 |
| `Amplify`（增益） | 0–4 / 0.55 | Bloom 正向贡献在加回前的能量增益；0 时节点恒等 |
| `Saturation`（饱和度） | 0–1.5 / 0.85 | 提取光源的颜色保留量；0 为中性亮度，1 保留源色，超过 1 会加强通道差异 |
| `Save lights`（保留高光） | 0–1 / 0.45 | 在原输入很亮的地方减少新增 Bloom，保护光芯；不关闭光源提取 |

#### 参数之间的区别

- `Threshold (EV)` 决定“哪些亮度能成为光源”。降低它会让更多弱窗、反光和噪点参与；提高它只留下更强的灯和高光。
- `Softness (EV)` 只决定阈值两侧的渐变宽度。它不是 `Radius` 的替代品，不能用来制造空间上的宽泛光。
- `Radius` 决定泛光向外扩散多远；`Amplify` 决定扩散贡献有多亮。想要“宽但不刺眼”，通常先增加 Radius，再用较低 Amplify 控制总量。
- `Saturation=0` 会用近似中性的亮度作为 Bloom 光源；`Saturation=1` 保留提取源的通道比例；更高值只是加强通道差异，不是色相旋转。
- `Save lights` 的作用是让原高光所在位置少叠加一些新增贡献，保留更清楚的光芯。它与下一节的 Highlight Protection 是两道不同位置的保护。
...
| 目标 | 可尝试的方向 |
|---|---|
| 自然、克制 | Threshold 2–3，Softness 1–2，Radius 0.4–0.8，Amplify 0.3–0.8 |
| 梦幻、宽扩散 | Radius 1–2，Softness 1.5–2.5，Amplify 0.6–1.2，并适当提高 Save lights |
| 保留彩色灯 | Saturation 0.9–1.2，先提高 Threshold，避免弱噪点被染色扩散 |

如果只想让某个曝光段产生泛光，在 Advanced 中设置 `Bloom output area`。这个遮罩只限制扩散贡献落在哪里，不改变哪些像素被提取成 Bloom 光源，也不改变 `bloomBase`。

### 5.3 Highlight Protection（高光保护）

#### 它保护的是什么

Highlight Protection 不重新提取高光、不重新模糊，也不修改源图。它读取最近一个前置 Bloom 发布的 `bloomBase` 与 `bloomContribution`，只在较亮的 Bloom base 上衰减新增贡献：

```text
T          = 0.18 × 2^ThresholdEV
protection = smoothstep(T, T × 2^SoftnessEV, max(bloomBase RGB))
keep       = 1 - Amount × protection
output     = bloomBase + bloomContribution × keep
```

这意味着它保护的是高光“芯”与原始色彩，不是把原始高光压暗。默认节点顺序中它位于 Bloom 后、Film Resolution/Grain 前；如果 graph 中有多个 Bloom，它消费的是其前面最近的那个 Bloom contribution。

#### 参数速查

| 参数 | 范围 / 默认值 | 作用 |
|---|---|---|
| `Amount`（数量） | 0–1 / 0.5 | 在保护区域内抑制 Bloom contribution 的强度；0 不产生保护，1 在遮罩为 1 时完全收回该贡献 |
| `Threshold (EV)`（阈值） | 0–8 / 2.5 | Bloom base 从哪个亮度开始进入保护；相对 0.18 的 EV |
| `Softness (EV)`（柔和度） | 0.1–4 / 1 | 保护从无到有的 EV 过渡宽度；越大越渐进 |

#### 怎么调

1. 先打开同一 graph 中位于前面的 Bloom，确定 Bloom 的 Radius、Amplify 和 Save lights；Highlight Protection 单独打开时不会凭空产生效果。
2. 再打开 Highlight Protection，从 `Amount=0.3–0.7` 起步。默认可把它的 Threshold 放在 Bloom Threshold 附近或略高的位置。
3. 如果灯芯仍被泛光冲淡，提高 `Amount` 或降低 Highlight Protection 的 `Threshold`；如果高光周围被压得太平，降低 `Amount` 或提高该 Threshold，并适当增加 Softness。
4. 如果希望只保护最亮的核心，提高 Highlight Protection 的 `Threshold`；如果希望保护更宽的高光区域，降低该 Threshold 或增加 Softness。原始高光本身不应因为此节点被压低；若整体变暗，应优先检查 Highlight Rolloff、效果层不透明度和混合模式。

当没有可用的前置 Bloom transient 时，节点保持输入不变，并记录 `missingBloomContribution` 警告；UI 会提示“Highlight Protection has no Bloom contribution. Enable Bloom to activate it.”。这与 Bloom `Amplify=0` 不同：Amplify 为 0 时 transient 存在，只是可见贡献为零。

#### 与 Bloom 的两道保护如何分工

| 控制 | 作用位置 | 适合解决 |
|---|---|---|
| Bloom `Save lights` | Bloom 生成最终贡献时，按当前输入亮度减少叠加 | 让光源原地的高光芯不要被自己的 Bloom 覆盖 |
| Highlight Protection | Bloom 完成后，按 `bloomBase` 的 EV 再衰减贡献 | 泛光已经铺开后，进一步保护较亮区域的原始层次 |
| `Highlight Rolloff` | 最终输出编码/写回前的软肩 | 防止接近 1 或超过 1 的输出突然剪切 |

实用顺序是：先用 Bloom 的 `Save lights` 保住最贴近灯体的核心，再用 Highlight Protection 控制高亮区域的泛光叠加。若两者都很强，画面可能显得过于收紧；如果只想保留中心光芯，优先调整 Save lights。

### 5.4 三个节点的效果区域（Luma mask）

三个模块都支持 Advanced 中的效果区域控制。`Apply to: Entire image` 等于全覆盖；选择 `Exposure range` 后，可设置 `Lower bound EV`、`Upper bound EV`、`Edge softness EV` 以及 `Inside/Outside EV range`。

遮罩使用节点输入的 canonical linear sRGB Rec.709 亮度：
...
上下边界分别用 smoothstep 形成渐变带，`Outside EV range` 会反转范围。对 Defringe 和 Highlight Protection，遮罩把节点输入与节点结果线性混合；对 Bloom，遮罩只缩放 `bloomContribution` 后重新合成，因此 Bloom 的光源提取仍按原输入进行。遮罩不改变 alpha，不裁剪 HDR，遮罩全为 0 时当前节点逐样本恒等。

## 6. Film Resolution / MTF
...
### 6.3 MTF response（MTF 响应）

该参数改变胶片材料的目标 MTF50。向右调高会保留更多高频细节；向左调低会增加柔化。它不是后期锐化 Amount，因此不会恢复源图中不存在的细节。

### 6.4 Shadow loss（暗部损失）与 Highlight loss（高光损失）

胶片在曝光曲线趾部和肩部的细节响应通常与中间调不同。Shadow loss 只额外影响较暗区域，Highlight loss 只额外影响较亮区域；中间调保持接近基础 Resolution loss。
...
### 7.3 Correlation（相关模式）

- `Analogue`：分别组合 fine、medium、coarse 三个相关尺度，结构最完整。
- `Fast`：把 fine 与 medium 合并为一个等效尺度并保留 coarse，减少计算量；颗粒仍由相同的确定性坐标随机源产生。

Fit 整图预览会采用速度优先路径；100% 原生像素检查与 Apply 使用当前选择的 Correlation 和渲染质量。需要最终输出时优先使用 Analogue，快速整图调参或批量工作可使用 Fast。

### 7.4 Amount（数量）、Size（尺寸）、Roughness（粗糙度）与 Chroma（色度）
...
- Fit 模式的显示底图最长边为 1024 px，完整画面以 `object-fit: contain` 显示。
- 光源提取来自最长边 2048 px 的效果代理，不是先把原图缩到 1024 px 再找光源。
- Source Expansion 使用 2048 px 代理尺度的 sigma。
- PSF 扩散使用 1024 px 显示尺度的 sigma。
...
- 首次打开插件、读取文档或绑定新源时，面板会自动发起预览，先显示色彩管理后的 1024 px 源图，并提示 `Source loaded. Refining film effects…`，随后替换为完整效果。文档轮询已登记首个活动文档，不会在约 750ms 后把同一文档误判为切换并清空图像。
...
- E>1 的高光按曝光档继续扩展，不会被压成与被裁剪为白色的高光完全相同；
...
Fit 预览是 1024 px，并固定使用速度优先路径，因此与全分辨率 Apply 可能存在细微差异。100% 模式读取原生裁片，按设备像素比点对点显示；Source 与 Preview 共用 Photoshop ICC 管理后的显示底板，避免左右整体明暗不一致，同时使用用户选择的 Diffusion/Correlation。它与 Apply 仍可能因面板 PNG 与 Photoshop 画布显示/输出位深不同而有极轻微量化差异。最终确认时可将插件 100% 的 Preview 与 Photoshop 100% 画布并排比较。
...
### 10.11 为什么名称是 Public Beta 1

当前插件的 package/manifest 与 graph 均为 1.7.0，公开名称使用 `Film Emulation V1.7 Public Beta 1` 来明确发布通道，并在界面和文档中标注为 Windows 测试版。自动门禁和本轮 Windows Photoshop 实机验收均已完成，可以进入 Public Beta 分发；由于仍需收集更多真实设备和工作流反馈，因此尚不称为正式稳定版。macOS 暂不在本 Beta 的支持声明内。

### 10.12 为什么 Defringe 打开后仍有紫边或绿边

Defringe 只在局部亮度边缘和 Cg 色差同时达到门控条件时校正，不会处理整块彩色物体或所有色差。请切到 100%/Actual，先确认 `Amount` 不为 0，再逐步降低 `Chroma threshold`；如果边缘没有被纳入，可略微提高 `Edge sensitivity` 或 `Radius`。若彩色细节被误伤，反向提高 Chroma threshold、降低 Edge sensitivity 或缩小 Radius。

### 10.13 为什么 Bloom 与 Highlight Protection 打开后变化很小

先确认 Bloom 的 `Amplify` 不为 0，并检查 `Threshold`、`Radius` 和 Bloom output area；阈值过高或遮罩为空时，Bloom contribution 本来就可能很少。Highlight Protection 只会衰减 Bloom contribution，不会单独生成泛光；如果希望看到更明显的扩散，先调整 Bloom，再用 Highlight Protection 收住高光芯。`Save lights` 与 Highlight Protection 是独立的两道保护，同时设得很高会让变化显得克制。

## 11. 推荐调参顺序速查

```text
1. Film format（胶片格式） + ISO（Resolution / Grain 共享）
...
19. Bloom Threshold (EV)（阈值（EV））/Softness (EV)（柔和度（EV）） + Radius (% diagonal)（半径（对角线百分比））
20. Bloom Amplify（增益） + Saturation（饱和度） + Save lights（保留高光）
21. 打开 Highlight Protection 后调 Amount（数量） + Threshold (EV)（阈值（EV））/Softness (EV)（柔和度（EV））
22. 各节点 Luma mask（Bloom output area / Protection area 等局部效果区域）
